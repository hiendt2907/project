...
# ── Enforce omni-audit-chain dynamic config (always alter — idempotent) ────────
# kafka-configs.sh --describe output format varies across broker versions and
# includes synonyms/sensitive fields that make naive awk parsing brittle.
# Using --alter unconditionally is safe: Kafka applies the config regardless of
# current state, and the operation is a no-op if the value is already correct.
echo "Enforcing omni-audit-chain config: cleanup.policy=compact retention.ms=${OMNI_AUDIT_CHAIN_RETENTION_MS}"
"${KUBECTL[@]}" exec -n "$NS" "deploy/$DEPLOY" -- \
...
echo "  [OK] omni-audit-chain cleanup.policy=compact retention.ms=${OMNI_AUDIT_CHAIN_RETENTION_MS}"
