...
| 13 | Dependency lockfile chưa wire vào image build — `requirements.lock` (pip freeze, Python 3.13.5) đã tạo 2026-07-03 trong Phase-1 Contract Freeze, nhưng `Dockerfile`/`Dockerfile.gateway` vẫn cài từ `requirements*.txt` range (`>=`) | Đổi Dockerfile sang lock cần rebuild + full runtime verify cả 3 deployment — slice riêng | Trung bình — build không reproducible cho tới khi wire |
| 14 | `ui/apps/provider-portal` fail type-check khi chạy dưới root `next build` (`@/lib/config` resolve theo root tsconfig thay vì tsconfig của app) — phát hiện 2026-07-03 khi rebuild `omni-ui`; đã exclude `apps`/`packages` khỏi root `ui/tsconfig.json` (chúng là workspace app riêng, có Dockerfile/tsconfig riêng) nên root build xanh trở lại, nhưng type error bên trong provider-portal vẫn tồn tại khi build app đó độc lập | Cần build/type-check `apps/provider-portal` riêng và sửa import `@/lib/config` cho đúng tsconfig của app — ngoài scope iteration Understanding UI | Trung bình — chỉ chặn khi rebuild image `aoip-provider-*` |
