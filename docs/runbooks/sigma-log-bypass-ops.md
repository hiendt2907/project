...
1. Set on **omni-fullstack** (`OMNI_WORKER_ROLE=full`) ConfigMap:
...
3. Verify: `kubectl logs deploy/omni-fullstack -n <ns> | grep log_surge_sigma_bypass_ok` after an `app_log` scenario with injected 5xx logs.
