...
**Fix:** Default **`E2E_REDIS_POD_AUTO=1`** patches `labels.pod` to the current `app=redis-exporter` pod in `multi-agent` (same idea as nginx auto-patch). Static pod names in the JSON go stale after ReplicaSet rollouts. Verify: `E2E_NGINX_POD_AUTO=0 bash scripts/gateway_alert_loki_verify.sh scripts/alert_payloads/alertmanager_business_sane.json`.
...
**Symptom:** `make e2e-incident-matrix` fails with `file or directory not found: tests/test_phase_b_app_log_replay.py` (exit 4).
**Fix:** `scripts/e2e_incident_matrix.sh` `_scenario_phase_b_pytest` referenced a deleted file. Updated to point to `tests/test_omni_stateful_loop.py` (three-lane stateful-loop suite, 70 tests). Verify: `make e2e-incident-matrix` → `all 9 scenarios passed`.

**Symptom:** `ExecutionResponse.resolution_state` semantics or `converged` flag unclear; or tests need to assert the correct exit branch per lane.
**Fix:** `resolution_state` values are exhaustive and mutually exclusive — `converged` (SDK verified healthy), `sigma_gate` (RESOURCE 3-sigma not anomalous → noop before LLM), `loki_unavailable` (APP_LOG fail-closed, no Loki evidence), `blocked` (INV_* invariant blocked mutation, terminal), `exec_skipped` (OMNI_ENV_MODE≠lab, execute skipped), `exhausted` (MAX_LOOP_ITERATIONS reached without healthy verify), `incomplete` (LLM chose noop or loop ended without SDK confirm). `converged=True` only when `resolution_state='converged'`. Verify: `pytest tests/test_omni_stateful_loop.py -q` (43 tests cover all branches per lane).
