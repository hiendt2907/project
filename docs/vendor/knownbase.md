...
**Fix:** `rollout_args_from_evidence_batch` resolves `deployment` from `workload` or `deployment_name`; `fault_rollout_deterministic_plan_from_batch` skips rollout when `_evidence_suggests_credential_failure`. **Lab chaos credential:** enable `OMNI_LAB_CHAOS_CREDENTIAL_AUTOFIX_ENABLED` + `OMNI_CHAOS_PG_APP_PASSWORD` (see `k8s/deployments/omni-worker-configmap.yaml`); `deterministic_mutate_plan_from_batch` merges `k8s_patch_secret` into the probe CSV allowlist when that flag is on so `chaos_credential_lab_autofix_plan_from_batch` runs **before** the LLM — expect log `probe_deterministic_mutate tool=k8s_patch_secret`. SDK ESCALATE path skips `_emit_suggest_remediation` when `OMNI_AUTO_EXECUTE_ENABLED=true`. Verify: `pytest tests/test_deterministic_mutate_from_evidence.py`; `bash scripts/inject_real_fault.sh` → `action_feedback_published … exit_code=0 status=ok`.
