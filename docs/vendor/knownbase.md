...
**Symptom:** Post-mutate verify fails only because **Prom** probes are INCONCLUSIVE (missing series) while clinical/SDK probes passed; or RAG upsert on **legacy** finalize without SDK verify when pilot expects verified-only experience.
**Fix:** `OMNI_SDK_VERIFY_OPTIONAL_PROBES` (default `prom_pod_cpu_cores,prom_pod_memory_wss`) — INCONCLUSIVE on those IDs does not fail verify; FAILED still fails. `OMNI_EXPERIENCE_REQUIRES_SDK_VERIFY=true` (default) skips `action_experience` upsert on `_finalize_feedback_success_legacy`; set `false` only for explicit lab rollback. PromQL tools: pass `deployment`/`workload` + `namespace` for workload-regex cAdvisor queries (`promql_workload_helpers` + `build_dynamic_promql`). Verify: `pytest tests/test_post_mutate_sdk_verify.py tests/test_autonomous_experience_gate.py tests/test_workload_promql.py`.

**Symptom:** Post-mutate verify shows `prom_pod_cpu_cores` / `prom_pod_memory_wss` **INCONCLUSIVE** or empty PromQL series after pod rollout — query still used **exact** `pod="<alert pod>"` while cAdvisor labels the new pod name.
**Fix:** `probe_prom_pod_cpu_cores` / `probe_prom_pod_memory_wss` use **workload-scoped** selectors: `pod=~"^<deployment|workload>-.*"` from labels (`deployment`, `workload`, `statefulset`, `gigo_metadata`) or derived from ReplicaSet-shaped pod name / STS ordinal — never exact alert pod. Helpers: `workload_pod_prefix_for_promql`, `promql_workload_pod_regex_selector` in `diagnostic_resource.py`. Verify: `pytest tests/test_workload_promql.py`.

**Symptom:** LLM-first credential remediation (`CrashLoopBackOff` + `password authentication failed`) loops on the same read-only call (`k8s_get_logs` / hallucinated `k8s_get_secret`) until `agentic_mutate_plan_fail`, then falls back to `SUGGEST_REMEDIATION`.
**Fix:** Added hard repeat guard in `run_agentic_mutate_plan` (repeat threshold=2 by `(tool,args_sha)`): block duplicate read-only execution, inject forced decision instruction (`mutate` or `escalate`), and abort early with explicit loop-guard trace labels. Added read-only secret-structure tools `k8s_get_pod_secret_refs` + `k8s_get_secret_keys` (names/keys only, no values) so planner can collect `secret_name/key` safely before mutate. Precondition gate now enforces required evidence (`secret_ref_confirmed`, `credential_source_of_truth`) and de-stales planner-reported missing fields after discovery succeeds; `k8s_patch_secret` now carries provenance fields `value_source` + `value_source_ref`. Verify: `pytest tests/test_agentic_planner_early_exit.py tests/test_feedback_full_agentic_planner.py tests/test_k8s_cluster_tools.py tests/test_k8s_sdk_tool_surface.py`.

**Symptom:** `gateway_alert_loki_verify.sh` with `alertmanager_business_sane.json`: SDK probes show **missing_pod** / HTTP 404 for `labels.pod`, INCONCLUSIVE evidence, weak proof gate.
**Fix:** Default **`E2E_REDIS_POD_AUTO=1`** patches `labels.pod` to the current `app=redis-exporter` pod in `multi-agent` (same idea as nginx auto-patch). Static pod names in the JSON go stale after ReplicaSet rollouts. Verify: `NS=multi-agent E2E_NGINX_POD_AUTO=0 bash scripts/gateway_alert_loki_verify.sh scripts/alert_payloads/alertmanager_business_sane.json`.

**Symptom:** Chaos `inject_real_fault.sh` / `KubePodCrashLoopVictim`: only `SUGGEST_REMEDIATION` + `omni_actions_audit_only`, or LLM `agentic_mutate_plan_fail`; or **`k8s_rollout_restart` instead of fixing stale DB password** (credential drift).
**Fix:** `rollout_args_from_evidence_batch` resolves `deployment` from `workload` or `deployment_name`; `fault_rollout_deterministic_plan_from_batch` skips rollout when `_evidence_suggests_credential_failure`. **Preflight (no workload hardcoding):** when logs indicate credential failure, `merge_preflight_deployment_secret_refs` (`evidence_consumer` → `pkg/reasoning/preflight_deployment_secret_refs.py`) reads the **Deployment** via Kubernetes API using **namespace+deployment from Prometheus alert labels**, appends probe `preflight_deployment_secret_refs` with **secretName/key paths only** (no Secret values). LLM/RAG then see structured refs in the Fact table before planning. **Production / real cluster:** `OMNI_ENV_MODE=prod` strips lab chaos credential autofix; use planner + RAG + SDK verify. Optional local chaos drill: `OMNI_LAB_CHAOS_CREDENTIAL_AUTOFIX_ENABLED` (never in prod ConfigMaps). Verify: `pytest tests/test_preflight_deployment_secret_refs.py` + post-mutate tests.

**Symptom:** `make e2e-incident-matrix` fails with `file or directory not found` for a deleted test file (exit 4).
**Fix:** `scripts/e2e_incident_matrix.sh` `_scenario_phase_b_pytest` now runs the full `tests/` suite (ignoring integration + real_services). Verify: `make e2e-incident-matrix` → all scenarios passed.

**Symptom:** Lab `nginx-test` + `gateway_alert_loki_verify.sh` / `alertmanager_nginx_waiting_fault.json`: prober log **404 pod `nginx-test-DYNAMIC` (NotFound)** — probe không chạm workload lỗi.  
**Fix:** `E2E_NGINX_POD_AUTO=1` để script patch `labels.pod` theo pod thật (`app=nginx-test`); tránh RollingUpdate **hai** pod (một Running + một stuck) → **scale deployment 0 rồi 1** trước POST. Script: [`scripts/lab_nginx_test_missing_configmap_e2e.sh`](../../scripts/lab_nginx_test_missing_configmap_e2e.sh). Báo cáo: [`docs/reports/lab_nginx_missing_configmap_e2e.md`](../../docs/reports/lab_nginx_missing_configmap_e2e.md).

**Symptom:** Cần taxonomy nhất quán cho alert → Redis → RAG experience; hoặc `k8s_apply_rbac_least_privilege` fail vì `executor_sa` không khớp SA thực tế trên binding cluster-admin.  
**Fix:** [`docs/vendor/OMNI_LABEL_SCHEMA.md`](OMNI_LABEL_SCHEMA.md) + [`config/omni_label_schema.yaml`](../../config/omni_label_schema.yaml). `security_probes.probe_k8s_rbac_drift` sets `executor_sa` from ClusterRoleBinding subjects; defaults `OMNI_EXECUTOR_SA` / tool defaults align to `omni-worker`. Inject script: [`scripts/inject_self_remediation_alerts.py`](../../scripts/inject_self_remediation_alerts.py) adds `omni.io/symptom-group`, `omni.io/layer`, `omni_verify_required`. Verify: `pytest tests/test_alert_identity.py tests/test_diagnostic_policy.py`.

**Symptom:** Debug inject/Loki không khớp `trace_id`; hoặc inject **nhiều** alert liên tiếp thì analyst Kafka **lag** (chỉ thấy trace đầu trong log).  
**Fix:** Gateway [`src/gateway/api.py`](../../src/gateway/api.py) dùng `X-Omni-Trace-Id` hoặc `?trace_id=` nếu khớp `[a-zA-Z0-9_-]{8,128}`; không thì `gw-prom-{12hex}`. [`scripts/inject_self_remediation_alerts.py`](../../scripts/inject_self_remediation_alerts.py) in `trace_id` từ JSON response; chạy **từng** `--only` hoặc nghỉ giữa POST — analyst xử lý `omni-diagnostic-evidence` **tuần tự**. Ưu tiên: `kubectl exec -n multi-agent deploy/omni-prober -- python3 /app/scripts/inject_self_remediation_alerts.py --only rbac` (tránh port-forward host flake). Verify: `pytest tests/test_gateway_trace_id.py`; Loki: `{namespace="multi-agent", pod_name=~"...|omni-analyst|..."} |= "<trace_id>"`.

**Symptom:** Cần đóng vòng Prober → Analyst → Executor → Feedback **đến khi drift/alert condition hết** và chỉ ghi RAG khi **state** khớp thực tế; không chỉ tin executor `exit_code=0` / kubectl success.  
**Fix:** `store_autonomous_trace_context` lưu `verify_probe_ids` + `anomaly_event_min`. Khi `OMNI_POST_MUTATE_VERIFY_PLANNER_ENABLED=true` (default): sau cooldown `OMNI_VERIFY_DELAY_SEC`, `run_verify_probes` → audit `ActionRecord` `kind=post_mutate_verify` → `run_post_mutate_state_verify_planner` (LLM phải `phase: done` + `resolution_summary` so với fresh facts / `<INITIAL_SYMPTOM>`) rồi mới `VERIFIED_SUCCESS`. Nếu probes vẫn FAILED hoặc planner không kết luận recovery → retry deterministic / full agentic / tombstone (`POST_MUTATE_STATE_VERIFY_NO_DONE`, `STATE_VERIFY_MAX_ATTEMPTS`, …). Chỉ SDK không qua LLM: `OMNI_POST_MUTATE_VERIFY_PLANNER_ENABLED=false` (hành vi cũ: probe PASSED + deployment gate → verify). Tắt probe hẳn: `OMNI_POST_MUTATE_SDK_VERIFY_ENABLED=false`. Verify: `pytest tests/test_feedback_full_agentic_planner.py tests/test_omni_self_learning.py`.

**Symptom:** Alert security self-remediation (`rbac_drift` / `configmap_security_drift`) không bao giờ tới analyst `DIAGNOSED` hoặc không emit `EXECUTE_MUTATE`; hoặc `proof_lane=resource` + sigma chặn dù probe đã FAILED.  
**Fix:** `evidence_batch.append_evidence_and_take_flush_batch`: non-workload nhưng có `register_diag_expected_probes` → flush khi `expected_set <= keys` (một probe không deadlock). `resolve_proof_lane`: ưu tiên `probe_structured_remediation_ready` **trước** matrix. `deterministic_mutate_from_evidence`: parse `extracted_fact` JSON string (do `coerce_evidence_dict`). Executor: `RbacV1Subject` thay `V1Subject` (kubernetes-asyncio). CSV tools: `OMNI_PROBE_DRIVEN_MUTATE_TOOLS` / `omni_probe_driven_mutate_tools`. Verify: `pytest tests/test_evidence_batch_security_flush.py tests/test_diagnostic_policy.py`; inject `--only rbac` → log `probe_deterministic_mutate` + `action_emitted action=EXECUTE_MUTATE`.
...
**Fix:** **infra_blocker** — kiểm tra CRI/Kubernetes provider (OrbStack/Docker Desktop), kube-apiserver ↔ kubelet exec; thử `kubectl exec` từ máy khác hoặc nâng cấp runtime. Workaround tạm: chạy POST alert từ pod có shell trong cluster theo cách khác (Job `curl`, port-forward tới gateway) — không sửa bằng đổi tên container nếu manifest đã đúng. Verify khi exec ổn: `kubectl exec … -- true` = 0 rồi `NS=multi-agent SCENARIOS=nginx_waiting_fault bash scripts/e2e_incident_matrix.sh`.
...
**Fix:** Pin model profile trực tiếp trong `k8s/deployments/omni-worker-configmap.yaml`: `OMNI_CHAT_MODEL=qwen3.6`, `OMNI_MODEL_REASONING_ENGINE=deepseek-r1:8b`, `OMNI_MODEL_HEAVY_LIFTER=gemma3:27b`, `OMNI_MODEL_HELPER=qwen3.6`, `OMNI_DIAG_EVIDENCE_LLM_MODEL=deepseek-r1:8b`, `OMNI_AUTONOMOUS_DECIDER_MODEL=deepseek-r1:8b`, `OMNI_EMBED_MODEL=nomic-embed-text:latest`. Verify: `ollama list`, `kubectl exec deploy/omni-analyst -- env | grep OMNI_.*MODEL`, `NS=multi-agent SCENARIOS=nginx_waiting_fault bash scripts/e2e_incident_matrix.sh`.
...
| ---------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `scripts/gateway_alert_loki_verify.sh`               | **Đã cập nhật:** `exec` mặc định `**deploy/omni-prober**` (`E2E_EXEC_DEPLOY`); log + Loki gom split + gateway. **`NS` bắt buộc** (không default trong script).                                                    |
| `scripts/follow-trace.sh`                            | **Đã cập nhật:** quét deploy split (+ `omni-worker` nếu scale>0); `FOLLOW_TRACE_DEPLOYS` override.                                                                |
| `scripts/proactive_e2e.sh`                           | **OK** — rollout cả split + gateway; `omni-worker` optional; **`NS` bắt buộc** (Makefile lab: `NS=multi-agent`).                                                                                                    |
...
**Symptom:** Chaos loop treo ở bước verify dù executor đã publish `event=action_feedback_published`; `omni-analyst-action-feedback` group không nhận partition (hoặc partition bị pod legacy `omni-worker` giữ), nên `autonomous_feedback_loop` không chạy được `STATE_MACHINE_VERIFIED`.  
**Fix:** Đảm bảo split topology sạch trước loop: scale `deployment/omni-worker` về 0 và xoá pod legacy còn treo nếu cần; `make ensure-kafka-topics`; restart `omni-analyst`; xác nhận `kafka-consumer-groups --describe --group omni-analyst-action-feedback` cho thấy partition `omni-action-feedback` thuộc consumer của analyst hiện tại.
...
**Symptom:** Pod stuck in `Pending` with `FailedMount: configmap "X" not found` / `CreateContainerConfigError`; pipeline emits `SUGGEST_REMEDIATION` with `reason_code=ERR_REA_SIGMA_GATE_BLOCKED` instead of `EXECUTE_MUTATE(k8s_create_or_patch_configmap)`.  
**Root causes (two, both fixed 2026-04-10/11):** (1) `k8s_describe_resource` schema only accepted `Pod|Deployment|Service` — LLM planner hit `ERR_REA_SCHEMA_VIOLATION` trying to describe the missing ConfigMap; (2) `_emit_agentic_mutate_if_any` fallback (`plan=None`) used `k8s_rollout_restart` unconditionally — no broken-spec check, so the tool was wrong and `proof_lane=resource` from matrix caused sigma-gate block (no metric spike for a missing CM).  
**Fix:** (1) Added `ConfigMap|Secret` to `DescribeResourceArgs` literal in `src/workers/k8s_cluster_tools.py`. (2) Added `evidence_suggests_broken_spec + CM name` check in fallback → selects `k8s_create_or_patch_configmap`. (3) Fallback now sets `fallback_lane_override="state"` so `blind_lane_eff="state"` reaches `_proof_of_fault_gate` — state lane fast-tracks without sigma. LLM path: planner guided by `_broken_spec_first_round_instruction`, should return `k8s_create_or_patch_configmap` + `proof_lane_hint=state`. Verify: `pytest tests/test_configmap_remediation.py` (25 tests); re-run `NS=multi-agent bash scripts/lab_nginx_test_missing_configmap_e2e.sh` after rebuilding image.
...
**Fix:** `scripts/proactive_e2e.sh` chờ vòng lặp `curl` :9090 trả 200 (tối đa ~60s) sau `rollout status` rồi mới chạy audit. Verify: `NS=multi-agent bash scripts/proactive_e2e.sh --skip-build`.
