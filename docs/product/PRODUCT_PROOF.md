...
- Last verified: 2026-07-02, iteration 1 của Continuous Productization Loop
...
| Continuous discovery (cust-edge, cust-db) | ✅ | ✅ | ✅ | ❌ | `omni:evdedup:staging-sim_cust-edge:{process_list,port_scan,service_topology,nfs_topology}` tồn tại trong Redis |
| Continuous discovery (cust-app) | ✅ | ✅ | ❌ **BROKEN** | ❌ | `omni:evrl:p:staging-sim_cust-app:*` chỉ có `remote_system_metrics`(PASSED)/`remote_log_errors`(FAILED) — không có `process_list`/`port_scan`/`service_topology` |
...
| Observation → Fact projection | ✅ (code từ `1bc6292`) | ✅ (sau redeploy iteration này) | ✅ | ❌ | log `onboarding_pipeline: system_model contradiction tenant=staging-sim` + Redis `omni:aoip:system_model:staging-sim` |
| System Twin persisted | ✅ | ✅ | ✅ **2/3 host** | ❌ (chỉ đọc được qua `redis-cli`) | `HGETALL omni:aoip:system_model:staging-sim` → revision=18, 57 facts (cust-edge 38, cust-db 19, **cust-app 0**) |
...
4. ✅ Inventory hiển thị 2/3 host (`cust-edge`, `cust-db`) qua Twin — **cust-app CHƯA có discovery data**.
5. ✅ Services/ports phát hiện đúng cho 2 host đã có data: `host:cust-db` → `exposes_port 3306` (mariadbd), `exposes_port 6379` (redis-server) — khớp `ss -lntp` chạy trực tiếp trên VM. `host:cust-edge` → `runs_process nginx` — khớp port 80 nginx trên VM.
6. ✅ Twin revision hiển thị và tăng theo thời gian thực (6 → 18 trong ~1 phút quan sát), provenance có `discovery:{probe}:{trace_id}` + `agent:unknown` (⚠️ `agent:unknown` là gap nhỏ — `to_observation()` không điền đúng `agent_id`, chỉ ghi placeholder).
...
1. **`cust-app` không có discovery probe nào** (`process_list`/`port_scan`/`service_topology`/`nfs_topology`) — chỉ có `remote_system_metrics`/`remote_log_errors`. Root cause CHƯA điều tra (ngoài scope iteration 1). Đây là bottleneck ưu tiên cho iteration 2.
2. **`agent:unknown` trong provenance** — `to_observation()` trong `src/aoip/onboarding_projection.py` không map đúng `agent_id` thật vào provenance string, làm giảm giá trị bằng chứng của Fact.
3. **Không có API/UI đọc Twin** — chỉ có thể verify qua `redis-cli HGETALL` trực tiếp. Chưa có endpoint operator-visible cho System Twin/Competency Matrix (P0 visibility gap theo Phase 11 của Master Prompt).
4. Kafka `PartitionCount=1` toàn hệ thống — chưa sửa (P1 riêng, xem drift-correction post-mortem).
