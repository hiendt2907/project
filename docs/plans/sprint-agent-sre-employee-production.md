...
|---|---|---|---|
| 1 | 0 raw content rời VM | ✅ **ĐÓNG 2026-07-07 (IT-1)** — agent 1.2.0 hash tại nguồn trên cả 3 VM; canary end-to-end proof + Redis sweep 0 leak (PRODUCT_PROOF Iteration 27). Baseline gốc: ❌ vi phạm sống tại `discovery_evidence.py:209-210` cả 3 VM, in-transit | `orb -m <vm> grep content_hash .../discovery_evidence.py`; `HGET omni:onboarding:doc:staging-sim doc_snapshot` → có mtime, không `content` |
| 2 | Drift phát hiện ≤1 heartbeat | ✅ **ĐÓNG 2026-07-07 (IT-2)** — bundle hash canonical 2 phía + release manifest; drill thật trên cust-db: tamper 1 dòng → heartbeat đầu tiên báo `drifted` + WARNING log; restore → `current` (PRODUCT_PROOF Iteration 28). Baseline gốc: ❌ không có cơ chế, VERSION khớp là tình cờ | `make publish-agent-release`; curl `/webhook/agent/versions` → `drift_status` per agent |
| 3 | Enroll VM mới không sửa tay | ✅ **ĐÓNG 2026-07-08 (IT-3)** — one-time enroll token (Admin API) → credential per-agent (PG `omni_admin.agent_credential`, sha256) → tenant binding; re-provision cust-app thật qua `scripts/enroll_remote_agent.py` không sửa tay, Twin có fact mới sau enroll; drill live: token dùng lần 2 → 401, revoke → 401 tức thì, agent key phát token → 403 (PRODUCT_PROOF Iteration 29). Baseline gốc: ❌ credential tĩnh trong `run.env`, không enroll token/revoke | `psql: SELECT * FROM omni_admin.agent_credential;` · `pytest tests/test_agent_enrollment.py` · curl drill enroll/revoke |
