"""Capability: ``systemd.journal_vacuum`` — M3 Human-approved Systemd Journal Vacuum.

Capability THỨ BA của VM/AOIP lane (sau ``systemd.restart_unit`` và
``systemd.reset_failed``). Cùng vertical slice sản phẩm, cùng khuôn mẫu:
Incident/Mission → evidence-backed Decision → approval required → typed
capability command → Agent preflight → guarded execution → verification →
structured outcome → Mission close/escalate → audit.

VÌ SAO capability này khác 2 cái trước: nó là capability ĐẦU TIÊN cho lane
SYS_RESOURCE (disk/tài nguyên đầy) — 2 capability trước đều thuộc lane
SYS_HARD_FAIL (unit down/failed). ``apply()`` chỉ chạy
``journalctl --vacuum-size=<target>`` — lệnh journald CHÍNH THỨC để dọn dữ
liệu journal cũ đến khi còn lại ``<target>`` — KHÔNG BAO GIỜ ``rm``/
``find -delete`` trực tiếp lên ``/var/log/journal``, nên blast radius bị giới
hạn đúng bằng những gì journald tự coi là an toàn để xoay vòng.

TARGET UNIT CỐ ĐỊNH: capability này luôn tác động lên chính unit
``systemd-journald.service`` (unit THẬT tồn tại trên mọi host systemd) —
journal vacuum về bản chất là hành động trên dữ liệu CỦA journald, nên khớp
tự nhiên với mô hình unit-scoped hiện có (canonical_scope, lease, allowlist
đều giả định target là 1 unit name) — KHÔNG cần tổng quát hoá lại
command_bridge/canonical_scope/RecoveryGate.scope_prefix cho một khái niệm
"target không phải unit".

Kiến trúc: build TRÊN cùng hạ tầng đã có, giống hệt
``systemd_reset_failed.py`` — KHÔNG viết lại lease/idempotency/audit/
tier_gate. Registry operator mới nằm ở ``aoip.recovery``
(``("disk_pressure_journal", "systemd")``), KHÔNG sửa executor chung.

Tái sử dụng TRỰC TIẾP (không hardcode lại) từ ``systemd_restart.py`` — vốn
generic, không đặc thù cho restart: ``SystemdRestartPolicy``/
``load_policy_from_env`` (CÙNG env var ``AOIP_ALLOWED_SYSTEMD_UNITS`` — một
allowlist unit chung cho mọi capability mutate unit đó, tránh đẻ thêm biến
môi trường mới; vận hành PHẢI thêm ``systemd-journald.service`` vào allowlist
để capability này chạy được), ``validate_unit_name`` (regex canonical unit
name không đổi theo capability), ``_unit_exists`` (LoadState check không đặc
thù restart), ``_classify_product_outcome`` (map RecoveryOutcome→product
taxonomy hoàn toàn dựa trên outcome.status/reason, không dựa vào capability),
và các hằng số ``OUTCOME_*``/``MODE_*`` (taxonomy sản phẩm dùng chung, không
đặc thù restart).

Ngưỡng disk-pressure (``AOIP_JOURNAL_VACUUM_THRESHOLD_BYTES``, default 2 GiB)
và target size sau vacuum (``AOIP_JOURNAL_VACUUM_TARGET_SIZE``, default
"200M") ĐỀU đọc từ env, KHÔNG hardcode trong code — xem
``aoip.recovery._journal_vacuum_threshold_bytes``/``_journal_vacuum_target_size``
(operator ``is_broken``/``capture_before``/``apply``/``health`` sống ở đó,
cùng chỗ với 2 operator trước, KHÔNG duplicate logic ở module này).

RISK: 0.12 — cao hơn ``reset_failed`` (0.10, chỉ dọn bookkeeping, KHÔNG bao
giờ đụng dữ liệu) một chút vì ``journalctl --vacuum-size`` THẬT SỰ xoá dữ
liệu (log journal cũ) — dù chỉ là log data, không phải app/process data hay
tiến trình đang chạy, nên vẫn thấp hơn nhiều so với ``restart_unit`` (0.30,
có rủi ro downtime thật). Vẫn đi qua đúng gate/tier_gate machinery, KHÔNG
bypass.
"""
...
from aoip.recovery import (
    ...
    RecoveryGate,
    RecoveryRequest,
    _journal_vacuum_target_size,
    plan_recovery,
)

CAPABILITY_NAME = "systemd.journal_vacuum"
CAPABILITY_VERSION = "1"

# Failure_mode/substrate registered in aoip.recovery.OPERATORS — mirrors how
# systemd_restart/systemd_reset_failed bind to their own (failure_mode, substrate).
_FAILURE_MODE = "disk_pressure_journal"
_SUBSTRATE = "systemd"
...
CAPABILITY_METADATA = {
    ...
    "risk_class": "low",                # deletes disposable journal log data only, never app/process state
    "blast_radius": "single_unit",
    "reversibility": "irreversible_log_data_deletion_only",
...
def build_typed_payload(
    *, mission_id: str, decision_id: str, incident_id: str, summary: str, unit: str,
    ...
    ``unit`` PHẢI là ``systemd-journald.service`` trong thực tế (target cố
    định của capability này) nhưng vẫn nhận qua param như 2 capability trước
    để giữ chữ ký uniform với ``command_bridge._CAPABILITY_ADAPTERS`` — unit
    hợp lệ khác vẫn qua được validate_unit_name/allowlist ở preflight, chỉ là
    vô nghĩa vận hành (journal vacuum của unit khác không tồn tại).
    """
    return {
        "capability": CAPABILITY_NAME,
        "capability_version": CAPABILITY_VERSION,
        ...
        "verification": {"require_below_threshold": True},
    }
...
def issue_capability_command(
    *, typed_payload: dict, approver: str, tenant: str, issued_at: float, expires_at: float,
    ...
    """Ký approval BINDING với payload_hash TẠI THỜI ĐIỂM issue rồi gói thành
    ...
    shape (mirrors ``systemd_reset_failed.issue_capability_command`` — see
    ADR-005): the deployed daemon's configured executor
    (``operations.build_recovery_executor``) only understands the "recovery"
    key, while the capability-specific CLI/console path (``_decode`` below)
    understands the typed shape — one payload works against either decoder.
    """
...
def _operator_summary(*, unit: str, mode: str, product_outcome: str, reason: str,
    ...
    next_step = {
        OUTCOME_EXECUTED_AND_VERIFIED: "Không cần hành động thêm — journal disk usage đã xuống dưới ngưỡng (verified).",
        OUTCOME_NO_ACTION_NEEDED: "Journal disk usage đã dưới ngưỡng trước khi vacuum — không có mutation nào chạy.",
        OUTCOME_BLOCKED_BY_POLICY: "Thêm systemd-journald.service vào AOIP_ALLOWED_SYSTEMD_UNITS nếu journal vacuum là chủ ý.",
        ...
        OUTCOME_EXECUTION_FAILED: "Kiểm tra log agent/transport — command journalctl --vacuum-size thất bại.",
        OUTCOME_VERIFICATION_FAILED: "Journal disk usage VẪN trên ngưỡng sau vacuum — cần điều tra thủ công (escalated).",
        OUTCOME_OWNERSHIP_LOST_AMBIGUOUS: "Xác minh thủ công disk usage journal — có thể 2 agent đã mutate.",
    ...
    return {
        ...
        "attempted": f"journal-vacuum {unit}", "target": {"unit": unit}, "mode": mode,
...
async def build_systemd_journal_vacuum_executor(
    *, redis, holder: str, transport, audit_log: audit.FileAuditLog, gate: RecoveryGate,
    ...
    Giống hệt cấu trúc ``systemd_reset_failed.build_systemd_reset_failed_executor``
    (cùng lease/ledger/fencing/tier_gate, KHÔNG bypass) — chỉ khác domain:
    mutation ở đây là ``journalctl --vacuum-size=<target>``, KHÔNG bao giờ
    start/stop/restart tiến trình và KHÔNG bao giờ xoá file trực tiếp.
    """
    ...
    async def executor(payload: dict) -> tuple[str, dict]:
        ...
        if mode == MODE_SHADOW:
            ...
            summary = _operator_summary(
                ...
                evidence={**evidence, "would_execute": f"journalctl --vacuum-size={target}",
                         "predicted_verification_plan": ["journalctl --disk-usage"]},
