...
ENTITY_APPLICABLE_FACETS: dict[str, frozenset[str]] = {
    ...
    "host": frozenset({"identity", "runtime_state", "process", "listening_ports", "monitoring", "logging"}),
}
...
def build_entity_competency(
    ...
    *,
    ...
    """Deterministic projection: same (model, contradictions, now, freshness_sec)
    always yields the same EntityCompetency. Never calls an LLM; never promotes
    CLAIMED to VERIFIED on its own."""
    ...
    for facet in FACET_SCHEMA:
        ...
        if facet == "identity":
            facets[facet] = (
                FacetValue(state=FacetState.VERIFIED, value=entity_id, confidence=1.0)
                if entity_id in model.known_nodes
                else FacetValue(state=FacetState.UNKNOWN)
            )
            continue
        if entity_type == "host":
...
async def build_entity_competency_from_store(
    ...
    log, then project. Read-only — never mutates the store."""
    ...
    return build_entity_competency(
        ...
        now=now, freshness_sec=freshness_sec,
    )
