"""Redis LIST pool + lease keys — khớp `ollama_max_concurrent` / OLLAMA_NUM_PARALLEL.
...
POOL_KEY = "omni:semaphore:{ollama}:pool"
POOL_KEY_PROACTIVE = "omni:semaphore:{ollama}:pool:proactive"
POOL_KEY_REACTIVE = "omni:semaphore:{ollama}:pool:reactive"
LEASE_PREFIX = "omni:semaphore:{ollama}:lease:"


class RedisOllamaSemaphore:
    ...
    async def init_pool(self) -> None:
        ...
        logger.info(
            "ollama_semaphore dual_lane init proactive_slots=%s reactive_slots=%s total=%s",
            self._n_proactive,
            self._n_reactive,
            self._max,
        )
    ...
    async def _acquire_single_pool(self, timeout_s: float, lane: str) -> str:
        ...
        while True:
            ...
            if time.monotonic() >= deadline:
                raise TimeoutError("ollama semaphore acquire timeout")
