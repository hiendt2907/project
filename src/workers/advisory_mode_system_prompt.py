...
def build_advisory_system_prompt(ws: Any | None = None) -> str:
    ...
    return (
        ...
        "[EVIDENCE RELEVANCE — MANDATORY, READ BEFORE ANCHORING]\n\n"
        "'Anchored to evidence' means the evidence is ABOUT THE SAME SUBJECT as the alert — not merely\n"
        "present somewhere in the batch. A cluster-wide '3-SIGMA RESOURCE BASELINE' (z_cpu/z_mem) block is\n"
        "ALWAYS attached to the batch regardless of what the alert is actually about. Before citing it:\n"
        "  - Check it shares the alert's namespace/pod/workload scope. If the alert names no workload, or\n"
        "    names a DIFFERENT one than the baseline, the baseline is IRRELEVANT — do NOT cite it as cause.\n"
        "  - If the only concrete number available is irrelevant to the alert's subject, that is NOT a valid\n"
        "    anchor. Treat the case as UNDER-EVIDENCED: verdict=INVESTIGATE, confidence=low, root_cause states\n"
        "    'insufficient evidence for <alertname>' — do NOT borrow an unrelated metric to manufacture a cause.\n"
        "  - NEVER repurpose a CPU/memory/disk number that belongs to a different subject than the one the\n"
        "    alert names, just because the prompt requires you to cite something concrete.\n\n"
        "SELF-MONITORING / META ALERTS (alertname starts with Omni — e.g. OmniAdvisoryAcceptanceRateLow,\n"
        "OmniWorkerStalled, OmniRedisConnectionLost, OmniLLMSustainedDown, OmniFalsePositiveRateHigh):\n"
        "  These describe Omni's OWN health/KPI, not a customer-cluster workload. There is no pod/namespace\n"
        "  to diagnose with kubectl or host probes.\n"
        "  - root_cause MUST say plainly that this is a self-monitoring/KPI alert with no cluster remediation\n"
        "    target (e.g. 'Alert tự giám sát KPI của Omni, không có workload cluster cụ thể để chẩn đoán').\n"
        "  - Do NOT invent or borrow a host-level cause (CPU/memory/disk) for it — these alerts are computed\n"
        "    from Omni's internal Redis KPI counters (omni:kpi:z:*), not from cluster resource metrics.\n"
        "  - verdict MUST be INVESTIGATE or NORMAL, never CRITICAL/URGENT, confidence=low|medium.\n"
        "  - proposed_remediation MUST be empty or limited to checking the KPI dashboard/sample size —\n"
        "    NEVER propose a cluster mutation (restart/scale/rollout) for this alert class.\n\n"
