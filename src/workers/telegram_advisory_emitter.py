...
def _render_verdict_header(advisory: AnalystAdvisory) -> str:
    ...
    lines = [
        f"{emoji} *Verdict:* {advisory.verdict}",
        f"🔍 *Root Cause:* {advisory.root_cause}",
        f"{_CONFIDENCE_EMOJI[advisory.confidence]} *Confidence:* {advisory.confidence}",
    ]
...
def _render_remediation_steps(steps: list[ProposedRemediationStep]) -> str:
    ...
    for step in sorted(steps, key=lambda x: x.order):
        ...
        lines.append(f"\n{approval_icon} *Step {step.order}:* {step.action}")
        if step.args:
...
def _render_forecast(forecast: ForecastTimeline) -> str:
    ...
    lines = [f"*📈 Impact Forecast ({forecast.method}):*"]
    if forecast.basis:
        lines.append(f"Basis: {forecast.basis}")
    if forecast.note:
        lines.append(f"⚠️ Note: {forecast.note}")
    lines.append("")
    for f in sorted(forecast.forecasts, key=lambda x: int(x.timeframe[:-1])):
        emoji = _SEVERITY_EMOJI.get(f.severity, "❓")
        lines.append(
            f"{emoji} *{f.timeframe}* [{f.confidence}]: {f.prediction}"
        )
    return "\n".join(lines)
...
async def render_advisory_to_telegram(
    ...
    if len(message) <= 4000:
        try:
            ...
            logger.warning("event=advisory_telegram_send_error chat_id=%s err=%s", chat_id, e)
    else:
        # Split into chunks
        ...
        for idx, chunk in enumerate(chunks):
            try:
                ...
                logger.warning(
                    "event=advisory_telegram_chunk_error chat_id=%s chunk=%s err=%s",
                    chat_id,
                    idx + 1,
                    e,
                )
...
async def render_advisory_batch_to_telegram(
    ...
    if len(message) > 3000:
        try:
            ...
            logger.warning("event=advisory_batch_summary_send_error chat_id=%s err=%s", chat_id, e)
        ...
        try:
            ...
            logger.warning("event=advisory_batch_send_error chat_id=%s err=%s", chat_id, e)
