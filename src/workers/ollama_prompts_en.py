"""English prompts for all Ollama chat calls (worker). Vietnamese UI may stay outside this path."""
...
def truncate_plain_text_to_max_words(text: str, max_words: int | None = None) -> str:
    ...
    if max_words is None:
        max_words = OLLAMA_MAX_OUTPUT_WORDS
...
K8S_TOOL_GUIDANCE_EN = (
    ...
    "- Pod known: `inspect_pod_deep`, `k8s_tail_logs`, `k8s_describe_resource` (Pod).\n"
...
SRE_JSON_GENERATOR_EN = (
    ...
    f"`reply.args.text` must be at most {OLLAMA_MAX_OUTPUT_WORDS} words."
)
...
SRE_JSON_GENERATOR_UNATTENDED_EN = (
    ...
    f"`omni_mark_resolved.args.summary` ≤ {OLLAMA_MAX_OUTPUT_WORDS} words (English)."
)
...
AGENTIC_REACT_RULES_EN = (
    ...
    f"≤ {OLLAMA_MAX_OUTPUT_WORDS} words).\n"
...
CONV_FALLBACK_SYSTEM_EN = (
    ...
    f"Keep total prose ≤ {OLLAMA_MAX_OUTPUT_WORDS * 3} words unless context requires more."
)
