...
def test_shell_fast_path_enabled() -> None:
    ...
    # god_mode is stripped when env_mode=prod (default); lab tests use dev.
    assert shell_fast_path_enabled(WorkerSettings(god_mode=True, env_mode="dev")) is True
    assert shell_fast_path_enabled(WorkerSettings(lab_unchained=True, env_mode="dev")) is True
...
def test_is_fast_path_auto_allowed_execute_shell_god_only() -> None:
    ...
    assert is_fast_path_auto_allowed("execute_shell_command", WorkerSettings(god_mode=True, env_mode="dev")) is True
    assert is_fast_path_auto_allowed("echo", WorkerSettings()) is True
    assert is_fast_path_auto_allowed("k8s_rollout_restart", WorkerSettings(god_mode=True, env_mode="dev")) is False


def test_fast_path_allowlist_union() -> None:
    base = fast_path_auto_execute_allowlist(WorkerSettings())
    god = fast_path_auto_execute_allowlist(WorkerSettings(god_mode=True, env_mode="dev"))
